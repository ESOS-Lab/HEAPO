...
void* stm_btree_insert_two( void * data ){ 
	int dat = *(int *)data; 	
	int ret_dat = dat* 30000 ; 	
	int i = 0 ; 	
	TM_INIT_THREAD ; 	
//	for( i = 300000 ; i < 300000+ret_dat ; i++){
...
//	for( i = 0 ; i < 100 ; i++) {
//		printf("insert [%d]\n" , i) ;
		//spos_btree_insert( obj_store , &i , &i , 8 ) ; 
//		lock() ; 
	//	printf("[%d]'s insertion %d\n", i, btree_sum++ ) ;	
//		m_lock(); 
		spos_btree_insert(obj_store , &static_key[i] , &static_key[i] , 0) ; 	
//		m_unlock() ;
//		unlock();
...
//	scanf("%d", &num) ; 	
	...
	if( num == 1){// normal btree insert
		...
		// TWO TIMES	
		for( i = 0 ; i < thread_num ; i++){
			if(pthread_create(&threads[i] , NULL , stm_btree_insert_two , (void *)&a[i])!=0){ 
				fprintf(stderr, "Error creating threads\n") ; 	
				exit(1);
			}
		}
