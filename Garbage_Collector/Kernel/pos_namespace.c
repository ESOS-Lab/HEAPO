...
insert_record_to_container:
	...
	spin_lock(&OID_lock);
	OID_counter++;
	record->Object_ID = OID_counter;
	spin_unlock(&OID_lock);
	printk("oid_counter = %d\n", OID_counter);
	printk("record->oid = %d\n", record->Object_ID);
	//dk e
