...
void pos_btree_traverse(unsigned long *input_node)
{
	int get_fill = 0;
	int i, j;
...
void test_gc(unsigned long *input_node)
{
	int get_fill = 0;
	int i, j;
	unsigned long* node=NULL;
	...
	for(i=0; i<get_fill; i++)
	{
		...
		else
		{
			...
			ret_ptr = pos_btree_lookup(Alloc_tree, key);
			if(ret_ptr == NULL)
			{
				printf("this key %d is garbage \n", *key);
			}
			else
			{	
				val_buf=bval(&btree_geo128, node, i);
				key_buf=bkey(&btree_geo128, node,i);
				printf("finded key = %d, value = %d\n", *key_buf, *val_buf);
			}
