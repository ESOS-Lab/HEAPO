...
void pos_ns_burst(struct pos_ns_trie_node *prev_trie_node, int prev_index)
{
	...
	new_trie_node = pos_meta_cache_alloc(&sb->pos_ns_node_cachep, POS_NS_TRIE_NODE);	//sslee
	...
	for (i=0; i<count; i++) {
		...
		if (record->str_length) {
			//record->str = (char *)pos_kmalloc(record->str_length);
				if(record->str_length < 32){
						record->str = (char *)pos_meta_cache_alloc(&sb->pos_ns_string_1_cachep, POS_NS_STRING_1);
				}else if(record->str_length >= 32 && record->str_length < 64){
						record->str = (char *)pos_meta_cache_alloc(&sb->pos_ns_string_2_cachep, POS_NS_STRING_2);
				}else if(record->str_length > 64){
						record->str = (char *)pos_meta_cache_alloc(&sb->pos_ns_string_3_cachep, POS_NS_STRING_3);
				}else{
				}
		...
		if (new_trie_node->ptrs[index]==0) {
			...
			container = pos_meta_cache_alloc(&sb->pos_ns_cont_cachep, POS_NS_CONTAINER);	//sslee
			container->head = NULL;
		...
		str_len = strlen(str);
		if(str_len < 32){
				pos_meta_cache_free(&sb->pos_ns_string_1_cachep, (void *)str, POS_NS_STRING_1);
		}else if(str_len >= 32 && str_len < 64){
				pos_meta_cache_free(&sb->pos_ns_string_2_cachep, (void *)str, POS_NS_STRING_2);
		}else{
				pos_meta_cache_free(&sb->pos_ns_string_3_cachep, (void *)str, POS_NS_STRING_2);
		}
	...
	pos_meta_cache_free(&sb->pos_ns_cont_cachep, (void *)burst_cont, POS_NS_CONTAINER); //sslee
}
		...
		const char *str, int str_length)
{
	...
	record = pos_meta_cache_alloc(&sb->pos_ns_rec_cachep, POS_NS_RECORD);	//sslee
	record->str_length = 0;
	record->str = NULL;
