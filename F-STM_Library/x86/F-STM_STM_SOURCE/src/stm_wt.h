 ...
 restart_no_load:
  if (LOCK_GET_OWNED(l)) {
    ...
    if (likely(tx->w_set.entries <= w && w < tx->w_set.entries + tx->w_set.nb_entries)) {
	  ...
	  #if HEAP_TINY_DELAEYD_CACHE_FLUSH_FLAGS == 0 
	  pos_clflush_cache_range( addr , sizeof(addr) ) ; 	
	  #endif
          return w;
        }
        if (prev->next == NULL) {
...
//  ATOMIC_FETCH_INC_FULL(&global_seq) ; 	
//  ATOMIC_STORE(&w->seq_num , global_seq) ; 	
