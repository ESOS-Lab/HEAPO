...
do_write:
  /* for commit/checksum flags setting */ 
  #if HEAP_TINY_FLAGS == 1
	#if HEAP_TINY_COMMIT_FLAGS == 1
		w->commit_mark = 0 ; 	
		COMMIT_PRINT("w->commit_mark[%d]\n", w->commit_mark) ; 	
	#endif 
	#if HEAP_TINY_CHECKSUM_FLAGS == 1 
		w->checksum_mark = 0x00 ; 	
		CHECKSUM_PRINT("w->checksum_mark[%p]\n", w->checksum_mark);
	#endif 
  #endif
  #if HEAP_TINY_GLOBAL_COUNT_FLAGS == 1 
  	struct pos_name_entry *name_entry ; 	
  	int index ;  
  	index = pos_name_table_index(tiny_obj_store) ; 	
  	name_entry = name_table[index] ; 	
  	ATOMIC_FETCH_INC_FULL(&name_entry->global_count) ; 	
	ATOMIC_STORE(&w->seq_num , name_entry->global_count) ; 
  #endif 
  #if HEAP_TINY_GLOBAL_COUNT_FLAGS == 0 
	/* if this flags set 0, declare variable on tinySTM */ 	
	ATOMIC_FETCH_INC_FULL(&gc) ; 	
	ATOMIC_STORE(&w->seq_num , gc) ; //only calculate on tinySTM 
  #endif

  /* Global Sequence Number */ 
/*  struct list_head *head; 	
  struct list_node *node ; 	
  head = (struct list_head *)pos_get_prime_object(tiny_obj_store);
  node = head->head ; 	
  while(node){
	if(*(int*)node->value < 60000 ){
		break;
	}else if(*(int*)node->value == 60000){
		ATOMIC_STORE(node->value , 0);	
		break; 
	}
	node = node->next ; 
  }
  ATOMIC_FETCH_INC_FULL(node->value);
  ATOMIC_STORE(&w->seq_num , *(int*)node->value);*/
...
//  void *arr[500] ; 	
  flush_cnt = 0 ; 
  w = tx->w_set.entries; 
/* performance degrade part */ 
#if HEAP_TINY_COMMIT_FLAGS == 1
  pos_clflush_cache_range( w, sizeof(w_entry_t)*tx->w_set.nb_entries); 
#endif 
  for( i = tx->w_set.nb_entries ; i > 0 ; i--,w++){ 
	#if HEAP_TINY_COMMIT_FLAGS == 1 
	COMMIT_PRINT("[%d]w->commit_mark[before][%d]\n",i,w->commit_mark) ; 	
	if(w->mask!=0){
		w->commit_mark = 1 ; 	
	}
	COMMIT_PRINT("[%d]w->commit_mark[after][%d]\n",i, w->commit_mark) ; 	
	#endif
	#if HEAP_TINY_CHECKSUM_FLAGS == 1 
	CHECKSUM_PRINT("[%d]w->checksum_mark[before][%p]\n", i , w->checksum_mark);
	if(w->mask!=0){
		calc_write_entry_checksum(w);	
	}
	CHECKSUM_PRINT("[%d]w->checksum_mark[after][%p]\n", i , w->checksum_mark);
	#endif 
  } 
  pos_clflush_cache_range( w, sizeof(w_entry_t)*tx->w_set.nb_entries) ; 
#endif

  w = tx->w_set.entries; /* initialize */ 
  for (i = tx->w_set.nb_entries; i > 0; i--, w++) {
    if (w->mask != 0){
      ATOMIC_STORE(w->addr, w->value);
      #if HEAP_TINY_DELAYED_CACHE_FLUSH_FLAGS == 0 	
      pos_clflush_cache_range(w->addr, sizeof(w_entry_t)) ; 
      #endif 
      /* KEONWOO DEBUG */
      if( w->addr == NULL ){ 
	     printf("w->addr = %p\n") ; 
...
# endif /* CM == CM_MODULAR */
  ...
  /* KEONWOO DEBUG */
#if HEAP_TINY_DELAYED_CACHE_FLUSH_FLAGS == 1 
  pos_clflush_cache_array( arr , flush_cnt ) ;
#endif 
 end:
  return 1;
}
