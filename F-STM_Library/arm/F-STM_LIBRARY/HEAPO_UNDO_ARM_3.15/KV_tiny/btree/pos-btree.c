...
// We have to turn off undo logging for performance test.
...
//	printf("\ncount n : %d\n" , n) ;
	...
	#if KEONWOO_DEBUG == 1
//	printf("return dest : %lu\n" ,dest) ; 
	printf("return *dest : %d\n" , *dest) ; 
	#endif 
	return dest;
}
...
void *bval(struct btree_geo *geo, unsigned long *node, int n)
{
#if MODE == 1
...
{
	#if KEONWOO_DEBUG == 1
	printf("\t[%s] pos_write_value\n", __func__ ) ; 	
	printf("\t[%s] &node[%ld] = %lu\n" , __func__ , geo->no_longs + n , &node[geo->no_longs+n]) ; 	
//	printf("\t[%s] *node[%ld] = %lu\n" , __func__ , geo->no_longs + n , *node[geo->no_longs+n]) ; 	
	printf(" val : %d\n" , val) ; 
	#endif 
...
void __btree_init(struct btree_head *head)
{
	head->node = NULL;
	head->height = 0;
	#if KEONWOO_DEBUG == 1 
...
int keycmp(struct btree_geo *geo, unsigned long *node, int pos,
		  unsigned long *key)
{
		...
		unsigned long *key, int level)
{
	...
	for (height = head->height; height > level; height--) {
		#if KEONWOO_DEBUG == 1 
		printf("[%s] height : %d , level = %d\n" ,__func__ ,  height , level ) ; 	
		#endif 
		for (i = 0; i < geo->no_pairs; i++)
			if (keycmp(geo, node, i, key) <= 0)
				break;
...
static int btree_grow(char *name, struct btree_head *head, struct btree_geo *geo)
{
	...
	if (head->node) {
		...
		#if KEONWOO_DEBUG == 1 	
		printf(" fill = %d\n" , fill) ; 	
		printf(" setkey and setval execute \n" ) ; 	
		#endif 
	}
#if CONSISTENCY == 1
	#if UNDO_CONSISTENCY == 1 
	...
	#else
	// update hashtable->node address to new node(node) 
	head->node = node ; 	
	head->height++ ; 
	#endif 
	pos_clflush_cache_range(head, sizeof(struct btree_head)); // Delayed flush
	#if KEONWOO_DEBUG == 1
	printf("pos_write_value\n") ; 	
	printf("name : %s\n" , name) ; 	
	printf("&head->node : %lu\n" , &head->node) ; 	
	printf("node:%lu\n" , node) ; 	
	printf("pos_write_value\n") ; 	
	printf("name : %s\n" , name) ; 	
	printf("&head->height : %lu\n" , &head->height) ; 	
	printf("head->height+1 : %d\n" , head->height+1) ; 	
	#endif
#else
	head->node = node;
	head->height++;
			      ...
			      void *val, unsigned long val_size, int level)
{
	...
	printf("[%s] head->height : %d\n" ,__func__ , head->height) ; 	
	printf("[%s]  level : %d\n" ,__func__ ,  level ) ; 	
	#endif
	//BUG_ON(!val);
...
retry:
	...
	#if KEONWOO_DEBUG == 1 
	printf("Getfill : %d\n" , fill) ; 	
	printf("fill : %d between geo->no_pairs : %d\n", fill , geo->no_pairs) ;
	#endif
...
//	#endif 
	...
	int rval;
//	printf("[%s]\n" , __func__) ; 	
//	printf(" %d %d %d\n" , *(unsigned long *)_key , *(unsigned long *)_val , val_size) ; 
