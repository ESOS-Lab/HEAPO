...
int stm_pos_btree_remove( char *name, void *_key){ 
	...
	if( name == NULL || _key == NULL ){ 
		printf("[%s] please check name or key value\n",__func__ ) ; 	
		return -1; 	
	}
	TM_START(2,RW); 	
	key = (unsigned long *)_key ; 	
	...
	if(head->height == 0){ 
		printf("[%s] head_height value is zero\n",__func__); 	
		return -1; 	
	}	
