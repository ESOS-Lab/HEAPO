...
static int set_add(intset_t *set, val_t val, thread_data_t * d)
{
  int result;
  node_t *prev, *next;
  val_t v;
...
# endif
   TM_START( 1 , RW ) ; 	
   prev = (node_t *)TM_LOAD(&set->head);//get the head ; 	
   TM_STORE( &set->head ,new_node(val,prev,1));
   TM_COMMIT; 
   /* TM_START(1, RW);
    prev = (node_t *)TM_LOAD(&set->head);
    next = (node_t *)TM_LOAD(&prev->next);
...
int main(int argc, char **argv)
{
 ...
 if ((threads = (pthread_t *)malloc( 10 * sizeof(pthread_t))) == NULL) {
    perror("malloc");
    exit(1);
  }
