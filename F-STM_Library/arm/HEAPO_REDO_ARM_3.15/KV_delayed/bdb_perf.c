...
static int rec_size = 8 ; 	
static int rec_num = 8 ;
...
void *bdb_hash_insert( void *data ){ 
	...
	for( i = 0+ret_dat ; i <per_thread_count+ret_dat ; i++){
		...
		for( j = 0 ; j < rec_size/8 ; j++ ){ 
			tmp_value[j] = rec_num + static_key[j] ; 	
		}
...
int main(int argc, char *argv[])
{
	...
	mode = atoi(argv[3]) ; 	
	if( argc != 4 ){ 
		printf("./bdb_perf obj_name thread_nu mode\n") ; 	
		printf(" mode1[hash] mode2[btree]\n") ; 
		return;
	}
