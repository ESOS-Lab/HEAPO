...
//¸®ÅÏµÇ´Â vma´Â »ðÀÔµÇ´Â vmaÀÇ ´ÙÀ½¿¡ À§Ä¡ÇÑ vmaÀÌ´Ù.
//pprev´Â »ðÀÔµÇ´Â vmaÀÇ ÀÌÀü¿¡ À§Ä¡ÇÑ vmaÀÌ´Ù.
//rb_parent´Â »ðÀÔµÇ´Â ³ëµå¸¦ °¡¸®Å°°Ô µÉ ºÎ¸ð³ëµåÀÌ´Ù.
...
//ÀüÃ¼ vmaµé·Î ±¸¼ºµÈ rb-tree, list¿¡ »ðÀÔ
...
//Æ¯Á¤ object storage¸¦ ±¸¼ºÇÏ´Â vmaµé·Î ±¸¼ºµÈ list¿¡ »ðÀÔ
...
//ÇØ´ç levelÀÇ map_array°¡ °¡¸®Å³ ¼ö ÀÖ´Â pageÀÇ °¹¼ö¸¦ ¹ÝÈ¯
...
//addr¸¦ Æ÷ÇÔÇÑ page¿Í ¸ÊÇÎµÈ page frameÀÇ number¸¦ ¹ÝÈ¯ÇÔ
	...
	unsigned long vm_start, unsigned long vm_end)
{
	...
	if (prev_vma && prev_vma->vm_end == vm_start &&
		...
		//case 1. prev_vma, next_vma¿Í merge
			...
			//nextÀÇ map_array ¿£Æ®¸®¸¦ prevÀÇ map_array¿¡ »ðÀÔ
	...
	unsigned long addr, unsigned long len)
{
	...
	if (vma->vm_start == addr) {
		...
		//case 2. ¼¼±×¸ÕÆ® ¾ÕºÎºÐ ÇØÁ¦
			...
			//ºÐ¸®µÉ map_array ºÎºÐÀ» »õ·Î¿î vmaÀÇ map_array¿¡ º¹»ç
	...
	//case 4. ¼¼±×¸ÕÆ® Áß°£ ºÎºÐ ÇØÁ¦
		...
		//ºÐ¸®µÉ map_array ºÎºÐÀ» »õ·Î¿î vmaÀÇ map_arry¿¡ º¹»ç
		first_pn = addr>>PAGE_SHIFT;
		last_pn = (addr+len)>>PAGE_SHIFT;
...
//»ç¿ëÀÚ ÁÖ¼Ò °ø°£¿¡ vm_area_struct¸¦ »ý¼º ÈÄ »ðÀÔ
...
asmlinkage void *sys_pos_seg_alloc(char __user *name, unsigned long len)
{
	...
	//ÇØ´ç °´Ã¼ÀúÀå¼Ò¸¦ ¸ÊÇÎÇÑ ¸ðµç ÇÁ·Î¼¼½º¿¡ »õ·Î¿î ¼¼±×¸ÕÆ® ¸ÊÇÎ½ÃÅ´
	prev_task_pid = &record->task_list;
	task_pid = record->task_list;
	while (task_pid) {
...
asmlinkage int sys_pos_seg_free(char __user *name, void *addr, unsigned long len)
{
	...
	//ÇØ´ç °´Ã¼ÀúÀå¼Ò¸¦ ¸ÊÇÎÇÑ ¸ðµç ÇÁ·Î¼¼½º¿¡¼­ ¼¼±×¸ÕÆ® ¸ÊÇÎ ÇØÁ¦½ÃÅ´
	prev_task_pid = &record->task_list;
	task_pid = record->task_list;
	while (task_pid) {
		pid = find_get_pid(task_pid->pid_nr);
...
//¿¿ ¿
//      obj_storage_name : ¿¿ ¿¿¿ ¿¿
//      type : ¿¿ ¿¿¿¿ ¿¿¿ ¿¿¿¿ ¿¿(Btree, Hashtable, Linked-List)
//      size : Key ¿¿ Value ¿¿¿
//      key : ¿¿ ¿ Key ¿¿
//      value : ¿¿ ¿ value ¿¿
...
//¿¿ : SFGC¿¿ ¿¿¿¿ Unmapped List¿¿ Unmap¿ 10¿(default)¿ ¿¿ ¿¿¿¿ ¿¿¿¿
//      ¿¿¿ ¿ ¿¿
//¿¿ ¿: vimtim_list¿ Unmap¿ 10¿¿ ¿¿ ¿¿¿¿ ¿¿¿ List¿ head
        ...
        if(sb->total_vm > 10)
        {
            gc_ptr = &(sb->gc_list);
                ...
                while(obj_count < 10 && gc_ptr->next != NULL)
                {
                        ...
                        list_move_tail(gc_ptr, &(sb->gc_list));
                        obj_count++;
                        gc_ptr = gc_ptr->next;
                }
        }
