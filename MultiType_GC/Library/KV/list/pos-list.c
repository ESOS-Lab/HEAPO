...
__thread unsigned int list_state=0;
...
//sb : lock for make node
//printf("[SB DEBUG : %s] before mutex lock\n", __func__);
...
//sb : insert node in pos list before allocating value
	node->next = head->head;
	head->head = node;
//sb : unlock for make node
//	pthread_mutex_unlock(&mutex);
#if CONSISTENCY == 1
	pos_clflush_cache_range(node->key, KEY_SIZE*sizeof(unsigned long));
#endif
//sb : unlock for make value
//	pthread_mutex_lock(&mutex);
...
//sb : unlock for make value
	pthread_mutex_unlock(&mutex);
//printf("[SB DEBUG : %s] after mutex unlock\n", __func__);
#if CONSISTENCY == 1
...
//      idx : 연결을 끊을 Linked List 내 인덱스
//출력 값: name에 해당하는 객체저장소가 없을 시 '-1' 반환, 정상적으로 수행 시 '0' 반환
int pos_list_delete(char *name)
{
	struct list_head *lh;
	struct list_node *cur_node, *tmp;
	int cnt=0;
	...
	while (cur_node != NULL) {
		tmp = cur_node;
		cur_node = cur_node->next;
		pos_free(name, tmp->value);
		pos_free(name, tmp);
		cnt++;
		cnt++;
	}
	//printf("%d chunk freed\n", cnt);
}
