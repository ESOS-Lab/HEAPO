...
//std::vector forward_list <struct  GVMeta *> GVMetalist ;
std::vector <struct  GVMeta *> GVMetalist ;
...
int thdsCount = 0;
struct  sdmprofile__Allocated_Stack_item * sdmprofile__Storethd_StactAlloPtr[100];
...
extern "C" {
    ...
    {
		if(sdmprofile__PF_init){return;}
		
		sdmprofile__PF_init = 1;
		//printf("\nsss:%d\n", sdmprofile__MainthreadNumber);
		//getwchar();
    ...
    void  sdmprofile__Embed_metadata_GV (U_64T index__,void * AtAddress , u_int32_t size_ , char * Name_string_ptr ,char * DataType_string_ptr , char * FilenameStringptr_  ,  char * FileDirStringptr_  , U_64T linenumber__ )
    {
        ...
        //GVMetalist.push _front(I);
		GVMetalist.push_back(I);
    ...
    void  sdmprofile__Embed_metadata_HP (u_int32_t index__, char * Estimated_Name_string_ptr ,char * DataType_string_ptr , char * FunctionName_string_ptr , U_64T linenumber__ , char * SourceFile__ , u_int32_t by )
    {
    ...
    void  sdmprofile__AddBBHitcount_HP(U_64T index__ ,U_64T BBhitcount ,int Rcount ,int Wcount)
    {
		...
		HPMetaArray [index__ ].r  =  HPMetaArray [index__ ].r + (Rcount * BBhitcount );
		HPMetaArray [index__ ].w  =  HPMetaArray [index__ ].w + (Wcount * BBhitcount );
    }
	...
	void *  sdmprofile__calloc(size_t nitems , size_t size  , u_int32_t  heap_index,u_int32_t thdnumber)
	{
		size_t size_ = (nitems * size);
    ...
    int sdmprofile__SearchRoutine_GVonly (void * AtAddress ,  u_int8_t  AccessType,u_int32_t thdnumber )
    {
    ...
    int sdmprofile__SearchRoutine_HPonly(void * AtAddress ,  u_int8_t  AccessType,u_int32_t thdnumber )
    {
        thds[thdnumber].Search++;
	...
	int sdmprofile__SearchRoutine_GVonlynoCache (void * AtAddress ,  u_int8_t  AccessType,u_int32_t thdnumber )
	{
		thds[thdnumber].Search++;
		GVMeta * HitItem ;
	...
	int sdmprofile__SearchRoutine_HPonlynoCache(void * AtAddress ,  u_int8_t  AccessType,u_int32_t thdnumber )
	{
		thds[thdnumber].Search++;
		struct  HeapAllocationMeta * loopptr ;
	...
	int sdmprofile__SearchRoutine_STonly (void * AtAddress ,  u_int8_t  AccessType , u_int32_t thdnumber, struct  sdmprofile__Allocated_Stack_item * ptr  )
    {
        thds[thdnumber].Search++;
		/*
		printf("\n");
	...
	void sdmprofile__SearchRoutine_ALL(void * AtAddress ,  u_int8_t  AccessType , u_int32_t thdnumber, struct  sdmprofile__Allocated_Stack_item * ptr  )
	{
		
		if(sdmprofile__SearchRoutine_HPonly(AtAddress,AccessType,thdnumber ) == 1){return;}
		if(sdmprofile__SearchRoutine_GVonly(AtAddress,AccessType,thdnumber) == 1){return;}
		if(sdmprofile__SearchRoutine_STonly(AtAddress,AccessType,thdnumber,ptr) == 1){return ;}
		
		
	}
	
	void sdmprofile__SearchRoutine_ALLnoCache (void * AtAddress_ ,  u_int8_t  AccessType_ , u_int32_t thdnumber_, struct  sdmprofile__Allocated_Stack_item * ptr_  )
	{
		void * AtAddress  = AtAddress_;
		u_int8_t AccessType = AccessType_ ;
		u_int32_t thdnumber = thdnumber_;
		sdmprofile__Allocated_Stack_item  * ptr = ptr_ ;
		
		if(sdmprofile__SearchRoutine_HPonlynoCache(AtAddress,AccessType,thdnumber ) == 1){return;}
		if(sdmprofile__SearchRoutine_GVonlynoCache(AtAddress,AccessType,thdnumber) == 1){return;}
		if(sdmprofile__SearchRoutine_STonly(AtAddress,AccessType,thdnumber,ptr) == 1){return ;}
    ...
    void sdmprofile__PrintResultBeforeAddingBBcount()
    {
        ...
        if (sdmprofile__profileAction == STACK_  || sdmprofile__profileAction == ALL_)
        {
            ...
            for (U_64T ix = 1 ; ix <  (sdmprofile__stackallocationCount +1); ix++)
            {
		...
		*/
        ...
        if (sdmprofile__profileAction == GV_ || sdmprofile__profileAction == ALL_)
        {
            ...
            for (auto &I : GVMetalist)
            {
                ...
                fprintf(f,"\n%lu,%lu,%lu,%u,%s,%s,%s,%s,%lu ", I->index, I->r, I->w, I->size, I->VarName, typestring.c_str() ,I->DirName,I->FileName,I->linenumber ) ;
            }
        ...
        if (sdmprofile__profileAction == HEAP_ || sdmprofile__profileAction == ALL_)
        {
            fprintf(f, "\nHeap Variable Memory Region Profile Result\n\nIndex,Read Count,Write Count,Pointer Name,Pointer Type,Function Name,Line Number,Source File,HitCount,Min Size,Max Size,By");
            ...
            for (  U_64T ix = 1 ; ix <  (sdmprofile__heapallocationCount +1)  ; ix++)
            {
				...
				std::string by = "";
				if ( HPMetaItem->by == 1) { by = "malloc" ; } else if (HPMetaItem->by == 2) { by = "calloc" ; }
				
                fprintf(f,"\n%u,%lu,%lu,%s,%s,%s,%lu,%s,%lu,%zu,%zu,%s", HPMetaItem->index , HPMetaItem->r,  HPMetaItem->w ,  HPMetaItem->EstimatedName, HPMetaItem->EstimatedDataTypeString ,  HPMetaItem->FunctionName ,  HPMetaItem->linenumber, HPMetaItem->SourceFile, HPMetaItem->hitcount, HPMetaItem->MinSize,
                        HPMetaItem->MaxSize , by.c_str()    ) ;
            }
            fprintf(f, "\n\n\n");
	...
	void sdmprofile__exit(int status )
	{
    ...
    {
		//printf("Ftn:FindThread\n");
		//getwchar();
		if(!sdmprofile__PF_init){sdmprofile__StartProfiling_Processes();}
	...
	struct  sdmprofile__Allocated_Stack_item * sdmprofile__FindStactAlloPtr(u_int32_t thd )
	{
		return sdmprofile__Storethd_StactAlloPtr[thd] ;
	}
	
	std::map<int , std::string> FntMap;
	void sdmprofile__Fnt(int fntnum , char * name_ )
	{
		FntMap[fntnum] = name_ ;
		printf("fnt name:%s\n" , FntMap[fntnum].c_str() );
	}
	
	//std::ostringstream oooooo;
	void sdmprofile__PP(char * fff  )
	{
		//printf("ftn:%s\n", fff);
		//oooooo << fff << "\n" ;
		FILE *f = fopen(  "call.csv"   , "a+") ;
		fprintf(f,"%s\n",fff  ); fclose(f);
	}
...
u_int32_t RegisterNewThread()
{
    ...
    if (sdmprofile__IncludeStackProfiling)
    {
		...
		sdmprofile__Storethd_StactAlloPtr[th->threadnumber] =  th->Stack_allocations_head;
    }
