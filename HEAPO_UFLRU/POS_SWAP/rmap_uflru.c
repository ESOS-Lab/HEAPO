		     ...
		     unsigned long address, void *arg)
{
	...
	} else if (PageAnon(page)) 
	{
		...
		{
			//printk("[POSDEBUG] %s PageAnon %d cnt %d map %d\n",__func__,page_to_pfn(page),page_count(page),page_mapcount(page));	
			swp_entry_t entry = { .val = page_private(page) };
			pte_t swp_pte;
			
			//printk("[POSDEBUG] PageAnon %d private %d, %p\n",page_to_pfn(page), entry.val, entry);	


			if (PageSwapCache(page)) {
				//printk("[POSDEBUG] PageAnon %d is SwapCache cnt %d map %d\n",page_to_pfn(page),page_count(page),page_mapcount(page));	
				/*
				...
				//printk("[POSDEBUG] PageAnon %d and migration is ok. cnt %d map %d\n",__func__,page_to_pfn(page),page_count(page),page_mapcount(page));	
				/*
			...
			//printk("[POSDEBUG] mm %p add %p pte %p swp_pte %p\n",mm,address,pte,swp_pte);	
			BUG_ON(pte_file(*pte));
		}
		else
		{
...
static int rmap_walk_anon(struct page *page, struct rmap_walk_control *rwc)
{
	...
	if(is_nvram(page_zone(page)))
	{
		...
		anon_vma_interval_tree_foreach(avc, &anon_vma->rb_root, pgoff, pgoff) {
			...
			printk("[POSDEBUG] page %d at vma %p and add %p 's %d circle count %d\n ",page_to_pfn(page), vma, address, count, page_count(page));
