				      ...
				      bool force_reclaim)
{
	...
	while (!list_empty(page_list)) {
		...
		printk("[POSDEBUG] %s %ld mapping %d writeback %d dirty %d\n",__func__,page_to_pfn(page),atomic_read(&page->_mapcount),PageWriteback(page),test_bit(PG_dirty, &(page)->flags));	
//		printk("[POSDEBUG] %s, %ld is ",__func__,page_to_pfn(page));	
		...
		if ((mapping && bdi_write_congested(mapping->backing_dev_info)) ||
                ...
                if (PageWriteback(page)) {
			printk("[POSDEBUG] %ld inside writeback",page_to_pfn(page));	
...
int __isolate_lru_page(struct page *page, isolate_mode_t mode)
{
	...
	if ((mode & ISOLATE_UNMAPPED) && page_mapped(page) && page_mapcount(page)>0)
			return ret;
