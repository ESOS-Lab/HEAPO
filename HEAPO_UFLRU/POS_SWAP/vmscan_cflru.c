			 ...
			 struct scan_control *sc)
{
	...
	{
		//printk("[POSDEBUG] %s page %d is not write_to_queue\n",__func__, page_to_pfn(page));
                return PAGE_KEEP;
	}
	...
	if (clear_page_dirty_for_io(page)) {
		...
		SetPageReclaim(page);
                ...
                if (res < 0)
		{
			//printk("[POSDEBUG] %s page %d is write error\n",__func__, page_to_pfn(page));
		...
		//printk("[POSDEBUG] %s page %d is clean\n",__func__, page_to_pfn(page));
...
////////////////////nyg
	...
	if(is_nvram(zone)){
		current->flags |= PF_KSWAPD | PF_SWAPWRITE;
//POS TEMP NYG//////////////////////////////////////////////////////////
