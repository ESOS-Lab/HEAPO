...
//#else
	name_entry->mstate = (void *)syscall(288, name, 4); // 4KB
//#endif
...
pos_map(char* name)
{
	...
	name_entry->mstate = (void *)syscall(290, name);
	if (name_entry->mstate == (void *)0) {
...
pos_seg_alloc(char *name, unsigned long len)
{
	...
	while (name_entry) {
		if (strcmp(name, name_entry->name) == 0) {
			...
			addr = (void *)syscall(292, name, len);
			if (addr == (void *)0) {
...
pos_seg_free(char *name, void *addr, unsigned long len)
{
	...
	while (name_entry) {
		if (strcmp(name, name_entry->name) == 0) {
			...
			if (syscall(293, name, addr, len) ==0) {
...
pos_is_mapped(char *name)
{
	...
	prime_seg = (void *)syscall(294, name);
	if (prime_seg == (void *)0) {
