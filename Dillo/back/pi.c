...
void print_all(struct timeval T1, struct timeval T2)
{
	long sec,usec;
	int pid;
	int thread_id;
	...
	time(&t);
	strftime(buf,128,"%H:%M:%S",localtime( &t));
	printf("%s",buf);
	printf("%10d %10d %11d %8ld sec %4ldus \n\n",pid,thread_id,priority,sec,usec);
	
}

void *main_function(int data)
{
      INDEXER x;
