...
// Web IO Time
unsigned long total_web_io_time, total_web_io_size;

// Decoding Time
unsigned long total_dec_time, total_dec_size;

// Disk IO Time
unsigned long total_disk_io_time, total_disk_io_size;

// Parsing Time
unsigned long total_parsing_size, total_parsing_time;
int cnt_space, cnt_tag, cnt_word;

// Drawing Time
//unsigned long total_draw_time;

struct timespec stamp_start, stamp_end;
...
// POS (Taeho Hwang)
printf("[POS] Web IO Time (total_size=%lu, total_time=%lu (nsec))\n", total_web_io_size, total_web_io_time);
printf("[POS] Decode IO Time (total_size=%lu, total_time=%lu (nsec))\n", total_dec_size, total_dec_time);
printf("[POS] Disk IO Time (total_size=%lu, total_time=%lu (nsec))\n", total_disk_io_size, total_disk_io_time);
printf("[POS] Parsing Time (total_size=%lu, total_time=%lu (nsec))\n", total_parsing_size, total_parsing_time);
printf("[POS] cnt_space=%d, cnt_tag=%d, cnt_word=%d\n", cnt_space, cnt_tag, cnt_word);

clock_gettime(CLOCK_REALTIME, &stamp_end);
printf("[POS] Init time (nsec=%ld\n", stamp_end.tv_nsec-stamp_start.tv_nsec);


total_web_io_time = total_web_io_size = 0;
total_dec_time = total_dec_size =0;
total_disk_io_size = total_disk_io_time = 0;
total_parsing_size = total_parsing_time = 0;
cnt_space = cnt_tag = cnt_word = 0;
