...
// POS (Taeho Hwang)
void pos_convert_str(char *dest, char *src, int len)
{
	int i;
...
fp = fopen("/root/dillo/dillo-2.2/WebPage/list", "rt");
if (fp==NULL) {
	return;
}
...
// POS (Taeho Hwang)
#if DISK_MODEL
entry->Flags |= CA_DiskRead;
entry->Flags &= ~CA_DiskWrite;

if (strstr(entry->Url->url_string->str, "gif")) {
	entry->TypeDet = dStrdup("image/gif");
	entry->TypeHdr = dStrdup("image/gif");
} else if (strstr(entry->Url->url_string->str, "jpeg")) {
	entry->TypeDet = dStrdup("image/jpeg");
	entry->TypeHdr = dStrdup("image/jpeg");
} else if (strstr(entry->Url->url_string->str, "pjpeg")) {
	entry->TypeDet = dStrdup("image/pjpeg");
	entry->TypeHdr = dStrdup("image/pjpeg");
} else if (strstr(entry->Url->url_string->str, "jpg")) {
	entry->TypeDet = dStrdup("image/jpg");
	entry->TypeHdr = dStrdup("image/jpg");
} else if (strstr(entry->Url->url_string->str, "png")) {
	entry->TypeDet = dStrdup("image/png");
	entry->TypeHdr = dStrdup("image/png");
} else if (strstr(entry->Url->url_string->str, "x-png")) {
	entry->TypeDet = dStrdup("image/x-png");
	entry->TypeHdr = dStrdup("image/x-png");
}
#endif
...
if (entry->Flags & CA_DiskRead) {
	...
	strcpy(file_name, "/root/dillo/dillo-2.2/WebPage/");
	pos_convert_str(url, URL_STR_(Url), Url->url_string->len);
	strcat(file_name, url);

	fd = open(file_name, O_RDONLY);
	r_length = read(fd, buf, sizeof(buf));
	buf[r_length] = '\0';

	entry->Data->str = dStrndup(buf, r_length);
	entry->ExpectedSize = entry->TransferSize = entry->Data->len = r_length;

	close(fd);

	clock_gettime(CLOCK_REALTIME, &stamp_e);
	total_disk_io_size += r_length;
	total_disk_io_time += stamp_e.tv_nsec-stamp_s.tv_nsec;
...
char *Cache_current_content_type(CacheEntry_t *entry)
{
...
if (entry->Flags & CA_DiskWrite) {
	...
	fp = fopen("/root/dillo/dillo-2.2/WebPage/list", "at");
	fprintf(fp, "%s\n", URL_STR_(Url));
	fflush(fp);
	fclose(fp);
	...
	write(fd, entry->Data->str, entry->Data->len);
	close(fd);
