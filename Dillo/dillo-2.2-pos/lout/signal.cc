...
//   receivers = new List <Receiver> (false);
   ...
   // POS (Changhun Jung)
   container::typed::List <Receiver> *_receivers;
   if (__IsPos == 1)
   {
      _receivers = new List <Receiver> (false, 1);
      //_receivers = new List <Receiver> (false);
	  receivers = (container::typed::List <Receiver> *)pos_malloc(__POS_OBJ_NAME, sizeof(container::typed::List <Receiver> *));
	  //receivers = (container::typed::List <Receiver> *)malloc(sizeof(container::typed::List <Receiver>));
	  memcpy(receivers, _receivers, sizeof(container::typed::List <Receiver>));
   }
   else
   	  receivers = new List <Receiver> (false);
...
void Emitter::connect (Receiver *receiver)
{
   ...
   #if (DEBUG == 1)
	   printf("   In Emitter::connect\n"); //POS (Changhun Jung)
   #endif
   //delete receiver->emitters; // POS (Changhun Jung)
   receiver->connectTo (this);
   #if (DEBUG == 1)
	   printf("   Emitter::connect -END- \n"); //POS (Changhun Jung)
   #endif
...
{
   //printf("Emitter::emitVoid \n"); // POS (Changhun Jung)
   //printf("&Emitter = 0x%lX\n", this); // POS (Changhun Jung)
   //printf("&receivers = 0x%lX\n", receivers); // POS (Changhun Jung)
   //printf("receivers's size =%d\n", receivers->size()); // POS (Changhun Jung)
   
   for (Iterator <Receiver> it = receivers->iterator (); it.hasNext (); ) {
      Receiver *receiver = it.getNext();
	  //printf("before emitToReceiver\n"); // POS (Changhun Jung)
      emitToReceiver (receiver, signalNo, argc, argv);
	  //printf("after emitToReceiver\n"); // POS (Changhun Jung)	  
   }
   //printf("Emitter::emitVoid -END-\n"); // POS (Changhun Jung)
}
