...
{
#if (DEBUG == 1)
	printf("(untyped)List::clear\n"); // POS (Changhun Jung)
#endif

#if 1

if(___mode == 0 || IsPos == 0) // POS (Changhun Jung)
{
...
{
   Node *newLast;
   Node *_newLast; // POS (Changhun Jung)
   
   // POS (Changhun Jung)
   if(___mode == 1 && IsPos == 1)
   {
		//_newLast = new Node;
		newLast = (Node *)pos_malloc(__POS_OBJ_NAME, sizeof(Node) );		
		//memcpy (newLast, _newLast, sizeof(Node) );		
   }
