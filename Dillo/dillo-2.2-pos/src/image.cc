...
DilloImage *a_Image_new(const char *alt_text, int32_t bg_color)
{
   ...
   //if( ___mode == 1) // POS (Changhun Jung)
...
*/
   ...
   else if( ___mode == 1)
   {
	   dw::Image *_image;
       dw::Image *__image;
	   __image = (dw::Image *) new dw::Image(alt_text);
	   _image= (dw::Image *)pos_malloc(__POS_OBJ_NAME, sizeof( dw::Image) );
	   memcpy (_image, __image, sizeof( dw::Image) );
	   Image->dw =(void*) _image;
   }
