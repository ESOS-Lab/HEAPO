...
void a_Cache_init(void)
{
   ...
   // POS (Changhun Jung)
   printf("In a_Cache_init!!!!!!\n");
   //fp_list = fopen("../list.txt","a+"); 
   fp_list = fopen("/root/dillo/dillo-2.2-pos/list.txt","a+"); 
   
   fseek(fp_list, 0L, SEEK_END);
   file_size = ftell(fp_list);
   printf("file size : %d\n",file_size);
   fseek(fp_list, 0L, SEEK_SET);
   ...
   /* inject the splash screen in the cache */
   {
	  ...
	  #endif
	  while( ftell(fp_list) != file_size )
	  {
	  		i = 0;
			while(1)
			{
				fread(&buf[i], 1, 1, fp_list);
				if(buf[i++] == '\0')
					break;
			}
	  ...
	  #endif
	  for(i=0; i<CachedURLs->len; i++)
	  {
			((CacheEntry_t*)(CachedURLs->list[i]))->_mode = 2;
	  }
	  // POS (Changhun Jung)
	  #if (DEBUG == 1)
   ...
   /* inject the splash screen in the cache */
   {
	  ...
	  if(___mode == 2)
	  {
		url = a_Url_new("http://www.daum.net", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
		
		url = a_Url_new("http://www.naver.com", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
		
		url = a_Url_new("http://www.google.co.kr", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
	
		url = a_Url_new("http://www.facebook.com/?_fb_noscript=1", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
	
		url = a_Url_new("http://us.yahoo.com/", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
	
		url = a_Url_new("http://www.aol.com/", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
	
		url = a_Url_new("http://www.usenix.com", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
	
		url = a_Url_new("http://www.dillo.org", NULL);
		ds = dStr_new(NULL);
		Cache_entry_inject(url, ds);
		dStr_free(ds, 1);
		a_Url_free(url);
...
static void Cache_entry_init(CacheEntry_t *NewEntry, const DilloUrl *Url)
{
   ...
   NewEntry->_mode = 1; // POS (Changhun Jung)
}
...
int a_Cache_open_url(void *web, CA_Callback_t Call, void *CbData)
{
   ...
   strcpy(__POS_OBJ_NAME, Url->url_string->str);
   printf("POS_OBJ_NAME : %s\n", __POS_OBJ_NAME);
#endif
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("before Cache_client_enqueue \n\n");
   #endif	 
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("after Cache_client_enqueue \n\n");
   #endif	    
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("after Cache_delayed_process_queue \n\n");
   #endif	    
	  ...
	  //fp_list = fopen("../list.txt","a+");	  
	  fp_list = fopen("/root/dillo/dillo-2.2-pos/list.txt","a+");	  
      strcpy(buf, Url->url_string->str);	  
	  fwrite(buf, strlen(buf) +1, 1, fp_list);
	  fclose(fp_list);
#endif
      ...
      if (Client->Url == entry->Url) {
         ...
         if (!Client->Callback) {
            ...
            if (TypeMismatch) {
			   ...
			   #if (DEBUG == 1)
			   		printf("[Cache_process_queue] after a_Web_dispatch_by_type\n");// POS (Changhun Jung)
			   #endif
               if (st == -1) {
...
static void Cache_delayed_process_queue_callback()
{
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("Cache_delayed_process_queue_callback! -END-\n\n");
   #endif
...
static void Cache_delayed_process_queue(CacheEntry_t *entry)
{
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("Cache_delayed_process_queue!  -END-\n\n");
   #endif
}
