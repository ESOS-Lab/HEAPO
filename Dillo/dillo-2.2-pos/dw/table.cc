...
/**
 * temp variables for pos malloc
 */
   // pos (Changhun Jung)
   lout::misc::SimpleVector<Child*> *_children;
   lout::misc::SimpleVector<core::Extremes> *_colExtremes;
   lout::misc::SimpleVector<int> *_colWidths;
   lout::misc::SimpleVector<int> *_cumHeight;
   lout::misc::SimpleVector<int> *_rowSpanCells;
   lout::misc::SimpleVector<int> *_colSpanCells;
   lout::misc::SimpleVector<int> *_baseline;
   lout::misc::SimpleVector<core::style::Style*> *_rowStyle;
   lout::misc::SimpleVector<float> *_colPercents;
...
{
   _children = new misc::SimpleVector <Child*> (16);
   _colExtremes = new misc::SimpleVector<core::Extremes> (8);
   _colWidths = new misc::SimpleVector <int> (8);
   _cumHeight = new misc::SimpleVector <int> (8);
   _rowSpanCells = new misc::SimpleVector <int> (8);
   _colSpanCells = new misc::SimpleVector <int> (8);
   _baseline = new misc::SimpleVector <int> (8);
   _rowStyle = new misc::SimpleVector <core::style::Style*> (8);   
   
 
   children = (lout::misc::SimpleVector<Child*> *)pos_malloc(__POS_OBJ_NAME,sizeof(lout::misc::SimpleVector<Child*>) );
   colExtremes =(lout::misc::SimpleVector<core::Extremes> *)pos_malloc(__POS_OBJ_NAME,sizeof(lout::misc::SimpleVector<core::Extremes>) );
   colWidths = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   cumHeight = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   rowSpanCells = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   colSpanCells = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   baseline = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   rowStyle = (lout::misc::SimpleVector<core::style::Style*> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<core::style::Style*>) );
  
   memcpy(children, _children, sizeof(lout::misc::SimpleVector<Child*>) );
   memcpy(colExtremes, _colExtremes, sizeof(lout::misc::SimpleVector<core::Extremes>) );
   memcpy(colWidths, _colWidths, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(cumHeight, _cumHeight, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(rowSpanCells, _rowSpanCells, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(colSpanCells, _colSpanCells, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(baseline, _baseline, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(rowStyle, _rowStyle, sizeof(lout::misc::SimpleVector<core::style::Style*>) );

   delete(_children);
   delete(_colExtremes);
   delete(_colWidths);
   delete(_cumHeight);
   delete(_rowSpanCells);
   delete(_colSpanCells);
   delete(_baseline);
   delete(_rowStyle);

   children->set_IsPos(1);
   colExtremes->set_IsPos(1);
   colWidths->set_IsPos(1);
   cumHeight->set_IsPos(1);
   rowSpanCells->set_IsPos(1);
   colSpanCells->set_IsPos(1);
   baseline->set_IsPos(1);
   rowStyle->set_IsPos(1);   
}
...
{   
   _colSpanCells = new misc::SimpleVector <int> (8);
   colSpanCells = (lout::misc::SimpleVector<int> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<int>) );
   memcpy(colSpanCells, colSpanCells, sizeof(lout::misc::SimpleVector<int>) );   
}
...
{
   _colPercents = new misc::SimpleVector <float> (8);	
   colPercents = (lout::misc::SimpleVector<float> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::misc::SimpleVector<float>) );
   memcpy(colPercents, _colPercents, sizeof(lout::misc::SimpleVector<float>) );
   colPercents->set_IsPos(1);
   delete(_colPercents);
}
...
void Table::sizeRequestImpl (core::Requisition *requisition)
{
   ...
   // pos (Changhun Jung)
   #if (DEBUG == 1)
	   printf("after forceCalcCellSizes\n!!\n");
   #endif   
...
void Table::getExtremesImpl (core::Extremes *extremes)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In Table::getExtremesImpl\n");
   #endif
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In Table::getExtremesImpl - END-\n");
   #endif
...
{
   // pos (Changhun Jung)
   //printf("In Table::addCell\n");
   
   Child *child;
   Child *_child; // pos (Changhun Jung)
   int colspanEff;
   ...
   for (int col = 0; col < colspanEff; col++)
      for (int row = 0; row < rowspan; row++)
      {
         if (!(col == 0 && row == 0)) {
			...
			{	
				_child = new Child ();
				child = (Child *)pos_malloc(__POS_OBJ_NAME, sizeof(Child) );
				memcpy (child, _child, sizeof(Child) );
				delete _child;
			}
...
void Table::forceCalcCellSizes ()
{
   ...
   // pos (Changhun Jung)   
   #if (DEBUG == 1)
	   printf("In Table::forceCalcCellSizes!!\n");
   ...
   // pos (Changhun Jung)   
   #if (DEBUG == 1)
	   printf("Table::forceCalcCellSizes -END- \n");
   #endif   
...
void Table::forceCalcColumnExtremes ()
{
   ...
   for (int c = 0; c < colSpanCells->size(); ++c) {
	  ...
	  //if(colSpanCells->size() == 5 && n < 0)
	  //if(colSpanCells->size() == 5 )
	  	//this->printAll();
