...
public:
   ...
   */
   ...
   inline void printAll()
   {
    	int Index;
		Child *child;
        printf("Table::printAll\n");

		//for (Index = 0; Index< children->size(); Index++)
		//{
			child = children->get(Index);
			(child->cell.widget)->printAll();
		//}
   ...
   // POS (Changhun Jung)
   inline void setChildLayout()
   {
   		int Index;
		Widget* child;
		for (Index = 0; Index < children->size(); Index++)
		{
			if (childDefined(Index))
			{
				#if (DEUBG == 1)
					printf("child [%d]\n",Index);
				#endif
				child = children->get(Index)->cell.widget;
				child->_setLayout();
				child->setChildLayout();
			}
   ...
   // POS (Changhung Jung)
   inline void setPlatform(dw::core::Platform *platform)
   {
   		int Index;
		Widget *child;
		#if (DEBUG == 1)
			printf("In Table::setPlatform\n");
		#endif
