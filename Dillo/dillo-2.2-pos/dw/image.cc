...
Image::Image(const char *altText)
{
   ...
   else if (___mode == 1)
   {
	   if (this->altText)
	   {
	   	this->altText = (char *)pos_malloc(__POS_OBJ_NAME, strlen(altText) + 1);
		memcpy(this->altText, altText, strlen(altText) + 1);
	   }
	   else 
	   	this->altText = NULL;
