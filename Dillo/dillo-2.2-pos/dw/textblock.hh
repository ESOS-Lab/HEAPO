...
public:
   ...
   //POS (Changhun Jung)
   inline void printWord(int wordIndex)
   { 
	//printf("In printWord\n");
   ...
   inline void printAll()
   {
    ...
    printf("Textblock::printAll\n");
	for(Index = 0; Index < words->size(); Index++)
	{
	    //printf("address of this: 0x%lX\n",(unsigned long)this);
	    word = words->getRef(Index);
	    if (word->content.type == core::Content::TEXT)
	    	printWord(Index);
	    else if (word->content.type == core::Content::WIDGET)
	    	(word->content.widget)->printAll();
	}	
   } 
	
   //POS (Changhun Jung)
   void setPlatform(dw::core::Platform *platform) 
   {   		
	    int Index;
		Word *word;
		...
		for(Index = 0; Index < words->size(); Index++)
		{
		    ...
		    else if (word->content.type == core::Content::WIDGET)
		    {
		    	// POS (Changhun Jung)
		    	#if (DEBUG == 1)
			    	printf(" this word is widget\n");
				#endif
			    	(word->content.widget)->setPlatform(platform);
   ...
   void setChildLayout()
   {
		...
		{
			word= words->getRef(Index);
			if(word->content.type == 8)
			{
				#if (DEBUG == 1)
					printf("word [%d]\n",Index);
				#endif
				(word->content.widget)->_setLayout();				
				(word->content.widget)->setChildLayout();
			}
		}
   }
