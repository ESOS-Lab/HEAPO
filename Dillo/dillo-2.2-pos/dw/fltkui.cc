...
void FltkResource::attachView (FltkView *view)
{
   ...
   if (view->usesFltkWidgets ()) {
      this->view = view;
	  	  #if (DEBUG == 1)
		  	printf("before create new widget\n");
		  #endif
	      widget = createNewWidget (&allocation);
	  	  #if (DEBUG == 1)
  		    printf("before view->addFltkWidget\n");
		  #endif
	      view->addFltkWidget (widget, &allocation);
		  #if (DEBUG == 1)
 		    printf("after view->addFltkWidget\n");	  
		  #endif
...
void FltkResource::sizeAllocate (core::Allocation *allocation)
{
   ...
   #if (DEBUG == 1)
	   printf("In FltkResource::sizeAllocate\n");
   #endif
   
   //attachView(platform->view);   
   //printf("getview\n");
   //printf(" &resource: 0x%lx\n",this);
   //printf(" &platform: 0x%lX\n",platform);
   view = platform->getView();
   #if (DEBUG == 1)
	   printf("after getview\n");
   #endif
                                                               ...
                                                               *allocation)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)	   
   	   printf("In fltkSpecificResource::sizeAllocate\n");
   #endif
   ...
   FltkSpecificResource <dw::core::ui::LabelButtonResource> (platform)
{
   ...
   else if(___mode ==1)
   {
	   ...
	   _length = strlen(label);
	   newstr = (char *)pos_malloc(__POS_OBJ_NAME, _length);
	   memcpy(newstr, label,_length);
	   newstr[_length] ='\0';
...
void FltkLabelButtonResource::sizeRequest (core::Requisition *requisition)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("In FltkLabelButtonResource::sizeRequest\n");// POS (Changhun Jung)
   #endif
...
void FltkComplexButtonResource::sizeAllocate (core::Allocation *allocation)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("In fltkComplexButtonResource::sizeAllocate\n");
   #endif
...
void FltkComplexButtonResource::setLayout (dw::core::Layout *layout)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In FltkComplexButtonResource::setLayout\n &flatView: 0x%lX \n",flatView);   
   #endif	   
                                                            ...
                                                            *allocation)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
   	printf("In FltkComplexbuttonResource::createNewWidget\n");
   #endif
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
   	printf(" before layout->attachView\n");
   #endif
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
   	printf("FltkComplexbuttonResource::createNewWidget -END-\n");
   #endif
   ...
   FltkSpecificResource <dw::core::ui::EntryResource> (platform)
{
   ...
   else if(___mode == 1)
   {
	   ...
	   {
	   	_length = strlen(label);
		this->label = (char *)pos_malloc(__POS_OBJ_NAME, _length + 1 );
		memcpy(this->label, label, _length +1 );
		this->label[_length] = '\0';
	   }
	   else
	   	this->label = NULL;
   }
   ...
   FltkSpecificResource <dw::core::ui::MultiLineTextResource> (platform)
{
   ...
   ::fltk::TextBuffer *_buffer;// POS (Changhun Jung)
   if(___mode == 0)// POS (Changhun Jung)
   ...
   {
	    _buffer = new ::fltk::TextBuffer;
		buffer = (::fltk::TextBuffer *)pos_malloc(__POS_OBJ_NAME, sizeof(::fltk::TextBuffer) );
		//printf(" sizeof(TextBuffer) : %d\n",sizeof(::fltk::Textbuffer) );
		if(buffer == NULL )
			printf(" buffer pos_malloc error!!!\n");
		memcpy (buffer, _buffer, sizeof(::fltk::TextBuffer) );
                                                            ...
                                                            *allocation)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In FltkMultiLineTextResource::createNewWidget \n");
   #endif
   ...
   // POS (Changhun Jung)
   // It should be removed and fixed
/*  
   char *new_text = (char *)malloc(sizeof(20) );
   new_text[0] = 'f';
   new_text[1] = 'u';
   new_text[2] = '\0';
*/   
   //printf(" before text->buffer (buffer) \n");
   //printf(" &buffer : 0x%lX\n",buffer);
   //buffer->text(new_text);   
   //printf(" &(buffer->text) : 0x%lX\n",buffer->text() );
   //printf(" buffer->text : %s\n",buffer->text() );
   text->buffer (buffer);
   printf(" after text->buffer (buffer) \n");   
   return text;
}
                                       ...
                                       *radioButtonResource)
{
   ...
   lout::container::typed::List <FltkRadioButtonResource> *_list;// POS (Changhun Jung)
   if(___mode == 0)// POS (Changhun Jung)
   ...
   // POS (Changhun Jung)
   	...
   	{
    	_list = new lout::container::typed::List <FltkRadioButtonResource> (false);
		list = (lout::container::typed::List <FltkRadioButtonResource> *)pos_malloc(__POS_OBJ_NAME, sizeof(lout::container::typed::List <FltkRadioButtonResource>) );
		memcpy (list, _list, sizeof( lout::container::typed::List <FltkRadioButtonResource> ) );
   	}
...
{  
   init (platform);
   Group *_group;// POS (Changhun Jung)
   ...
   if (groupedWith) {      
	  ...
	  else if(___mode == 1)
	  {
		  ...
		  group = (Group *)pos_malloc(__POS_OBJ_NAME, sizeof(Group) );
		  memcpy (group, _group, sizeof(Group) );
	  }
...
FltkSelectionResource<I>::createNewWidget (core::Allocation *allocation)
{
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In FltkSelectionResource::createNewWidget\n");
   #endif
   ...
   widgetStacks->append (widgetStack); // error occurs here
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In FltkSelectionResource::createNewWidget -END-\n");
   #endif
