...
int FltkViewBase::handle (int event)
{
   ...
   //printf("FltkViewBase::the Lyaout = 0x%lX\n",theLayout);
   //printf("the Lyaout->Widget = 0x%lX\n",theLayout->getRefWidget());
   //printf("FltkViewBase::view =0x%lX\n",this);

//if (___mode != 2)
//{

   //return Group::handle (event); // POS (Changhun Jung)
   
   // POS (Changhun Jung)
   
   #if (DEBUG == 1)
      printf("FltkViewBase::handle\n");
   #endif
   ...
   case MOVE:
      ...
      mouse_x = event_x();
      mouse_y = event_y();
	  //printf("x =%d, y =%d\n", mouse_x, mouse_y); // POS (Changhun Jung)
	  // POS (Changhun Jung) this is problem
      processed =
         theLayout->motionNotify (this,
                                  translateViewXToCanvasX (mouse_x),
                                  translateViewYToCanvasY (mouse_y),
                                  getDwButtonState ());
	  //printf("after layout->motionNotify\n"); // POS (Changhun Jung)
      _MSG("MOVE => %s\n", processed ? "true" : "false");
      return processed ? true : Group::handle (event);
                             ...
                             int x, int y, const char *text, int len)
{
 ...
 /**   
  * printf("&ff =0x%lX\n",ff);
  * printf("&ff->font =0x%lX\n",ff->font);
  * printf("ff->size =%d\n",ff->size);
  */   
  
   //setfont(ff->font, ff->size);// POS (Changhun Jung) 
   setcolor(((FltkColor*)color)->colors[shading]);   
                                  ...
                                  core::Allocation *allocation)
{
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf("after allocateFltkWidget\n");
   #endif
                                       ...
                                       core::Allocation *allocation)
{
   ...
   //MSG("In FltkWidgetView::allocateFltkWidget\n");// POS (Changhun Jung)
                                   ...
                                   core::Rectangle *area)
{
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" In FltkWidgetView::drawFltkWidget\n");
   #endif
   ...
   // POS (Changhun Jung)
   #if (DEBUG == 1)
	   printf(" FltkWidgetView::drawFltkWidget -END-\n");
   #endif   
}
